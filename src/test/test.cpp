...
int mainHelper(int argc, char *argv[], TLCM &roslcm)
{
    ...
    while (ros::ok()){
        ...
        if(motiontime>1000 && motiontime<40000){
            SendHighROS.mode = 1;
            SendHighROS.pitch = 0.5f;
            SendHighROS.yaw = 0.5f;
        }
        /*
                if(motiontime>2000 && motiontime<3000){
            SendHighROS.mode = 2;
            SendHighROS.forwardSpeed = -1.0f;
        ...
        /*
        if(motiontime>5000 && motiontime<10000){
            SendHighROS.mode = 1;
            SendHighROS.roll = -0.3f;
        }        
        if(motiontime>10000 && motiontime<15000){
            SendHighROS.mode = 1;
            SendHighROS.roll = 0.5f;
        }        
        if(motiontime>15000 && motiontime<20000){
            SendHighROS.mode = 1;
            SendHighROS.roll = -0.5f;
        }
